...
async function handleProfileRequest(req, res) {
  const profileUrl =  req.headers["x-profile-url"] || req.body?.profile_url;
  const cookie = req.headers["x-cookie"] || req.body?.cookie || null;
  const extractionMode = "llm";
...
app.post("/api/v1/profile", handleProfileRequest);
